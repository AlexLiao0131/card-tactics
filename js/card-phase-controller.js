...
function create(ctx){
  ...
  function waterRecheckUnits(units,reason){
    (units||[]).filter(unit=>unit?.alive).forEach(unit=>ctx.applyEnvironmentHazardToUnit(unit,{reason,waterTrigger:"CHANGE",includeElectric:false,includeBoiling:false,includeFire:false}));
  }
  ...
  function begin({initial=false}={}){
    ...
    if(cardState.zones.deck.length===0&&cardState.zones.hand.length===0){
      ...
      ctx.setPhase(PHASE.PLAYER);
      ctx.clearSelection();
      ctx.pushLog(`Round ${s.round}｜牌庫已抽完，跳過卡牌階段，直接進入戰棋階段。`,"SYSTEM");
      ctx.render();ctx.emitState();return;
    }
    ctx.setPhase(PHASE.CARD);
    ctx.clearSelection();
    const drawn=CardPhaseEngine.begin(cardState,{handSize:Number(s.stage.cardRules?.handSize||5)});
    ctx.pushLog(`Round ${s.round}｜卡牌階段開始｜💎 ${cardState.crystals}。`,"SYSTEM");
    if(drawn.length)ctx.pushLog(`抽牌 ${drawn.length} 張。`,"SYSTEM");
    ctx.setPendingCard(null);ctx.render();ctx.emitState();
  }

  function end(){
    const s=state();if(s.phase!==PHASE.CARD)return false;
    ctx.setPendingCard(null);CardPhaseEngine.end(s.cardState);ctx.setPhase(PHASE.PLAYER);
    ctx.pushLog(`Round ${s.round}｜進入戰棋階段。`,"SYSTEM");ctx.render();ctx.emitState();return true;
  }

  function select(cardId){
    const s=state();if(s.phase!==PHASE.CARD)return false;
    const card=CardDatabase.get(cardId);
    if(!CardPhaseEngine.canPlay(s.cardState,card))return false;
    if(CardDatabase.isCharacter(card)){
      ctx.setPendingCard(card);
      ctx.pushLog(`選擇 ${card.name}，請在亮起的我方部署區手動選擇出生格。`,"SYSTEM");
    ...
    if(card.effect?.type==="WEATHER"){
      ...
      ctx.resolveEnvironmentEvents?.(events,{reason:"天候造成水位／地表狀態變化"});
  ...
  function resolveAt(card,center){
    const s=state();
    if(!card||s.phase!==PHASE.CARD||ctx.getPendingCard()!==card)return false;
    ...
    if(effect.type==="AREA_FIRE"){
      ...
      affected.forEach(tile=>{
        const events=EnvironmentEngine.apply({map:s.map,state:s.environmentState,x:tile.x,y:tile.y,forces:effect.forces||["FIRE"]})||[];
      ...
      affected.forEach(tile=>{const u=ctx.unitAt(tile.x,tile.y);if(u)ctx.applyEnvironmentHazardToUnit(u,{reason:"遭野火波及",waterTrigger:"CHECK",includeElectric:false,includeBoiling:false,includeFire:true});});
      ...
      if(!window.HydrologyEngine?.floodArea){
        ...
        ctx.pushLog(`${card.name}｜注入 Water Volume ${Number(resolved?.injectedVolume||0).toFixed(2)}｜目標水面 H${resolved?.targetSurface??"?"}｜${wetCount} 格形成／加深水域。`,"SYSTEM");
        events.forEach(ctx.logEnvironmentEvent);
        ctx.resolveEnvironmentEvents?.(events,{reason:`${card.name} 造成水位重新分配`});
      }
    }
  ...
  function deployAt(tile){
    const s=state(),card=ctx.getPendingCard();
    if(!card||s.phase!==PHASE.CARD)return false;
    if(!DeploymentEngine.canDeploy({stage:s.stage,map:s.map,units:s.units,owner:"PLAYER",x:tile.x,y:tile.y}))return false;
    const unit=ctx.createUnit(ctx.nextUnitId(),TEAM.PLAYER,card.characterId,tile.x,tile.y);
  ...
  function cancel(){
    if(!ctx.getPendingCard())return false;
