window.DeathLifecycleEngine=(()=>{
  function create(ctx){
    ...
    function finalize(unit,source=null,cause=null){
      ...
      if(finalized.has(unit.id))return false;finalized.add(unit.id);
      ...
      if(unit.cardId&&cards){CardPhaseEngine.characterDefeated(cards,unit.cardId);ctx.pushLog(`${unit.character.name} 戰敗，角色卡進入墓地。`,"SYSTEM")}
