window.DeckEngine=(()=>{
  ...
  function normalize(ids=[]){const out=[];for(const id of ids){const card=CardDatabase.get(id);if(!card)continue;if(card.unitType==="HERO"&&out.includes(id))continue;out.push(id);}return out;}
  function autoBuild(groupId,{size=15}={}){const cards=PackDatabase.cards(groupId),heroes=cards.filter(c=>c.unitType==="HERO"),units=cards.filter(c=>c.type==="CHARACTER"&&c.unitType!=="HERO"),spells=cards.filter(c=>c.type==="SPELL"),out=[];heroes.slice(0,2).forEach(c=>out.push(c.id));let i=0;while(out.length<Math.min(size,10)&&units.length)out.push(units[i++%units.length].id);i=0;while(out.length<size&&spells.length)out.push(spells[i++%spells.length].id);i=0;while(out.length<size&&units.length)out.push(units[i++%units.length].id);return normalize(out);}
