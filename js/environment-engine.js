window.EnvironmentEngine=(()=>{
  ...
  const EFFECT={BURNING:"BURNING",BOILING:"BOILING",STEAM:"STEAM",FRAGMENTS:"FRAGMENTS",TORNADO:"TORNADO",FIRE_TORNADO:"FIRE_TORNADO",ELECTRIFIED:"ELECTRIFIED"};
  ...
  function applyRainToTerrain(map,state){
    ...
    return HydrologyEngine.applyRain(map,{heavy});
  }
  function advanceHydrology(map,state){
    if(!map||!state)return[];
    if(isRain(state))return applyRainToTerrain(map,state);
    return HydrologyEngine.drySoil(map,{source:"CLEAR_WEATHER"});
  }
  ...
  function addSteam(state,x,y,events,{duration=2,reason="HEAT"}={}){
    const existed=effectAt(state,x,y).some(e=>e.type===EFFECT.STEAM);
    addEffect(state,x,y,{type:EFFECT.STEAM,duration,visionBlock:true});
    if(!existed)events.push({type:"STEAM_CREATED",x,y,effect:EFFECT.STEAM,reason});
  }

  function heatWater(map,state,x,y,events=[]){
    const tile=tileAt(map,x,y);
    if(!HydrologyEngine.isWater(tile))return false;
    const existing=effectAt(state,x,y).find(e=>e.type===EFFECT.BOILING);
    const heat=Number(existing?.heat||0)+1;
    addEffect(state,x,y,{type:EFFECT.BOILING,duration:2,heat,damage:HAZARD.BOILING_DAMAGE,damageType:"FIRE"});
    addSteam(state,x,y,events,{duration:existing?2:1,reason:existing?"BOILING":"BOILING_START"});
    if(!existing){
      events.push({type:"WATER_BOILING",x,y,effect:EFFECT.BOILING,heat,waterDepth:waterDepth(tile)});
      return true;
    }
    const removed=removeWater(tile,1,events);
    events.push({type:"WATER_EVAPORATION",x,y,amount:removed,heat,waterDepth:waterDepth(tile)});
    HydrologyEngine.redistribute(map,{source:"EVAPORATION",events});
    if(!HydrologyEngine.isWater(tile)){
      removeEffect(state,x,y,EFFECT.BOILING);
      events.push({type:"WATER_BOILED_DRY",x,y});
    }
    return true;
  }
  ...
  function apply({map,state,x,y,forces=[]}){
    const forceSet=new Set(forces),events=[];
    const raining=isRain(state),burningBefore=isBurning(state,x,y),steamBefore=effectAt(state,x,y).some(e=>e.type===EFFECT.STEAM);
    ...
    if(forceSet.has(FORCE.WIND)&&steamBefore){
      removeEffect(state,x,y,EFFECT.STEAM);
      events.push({type:"STEAM_DISPERSED",x,y});
    }
    if(forceSet.has(FORCE.WIND)&&burningBefore){
      addEffect(state,x,y,{type:EFFECT.FIRE_TORNADO,duration:2,lightRadius:3,damage:HAZARD.FIRE_TORNADO_DAMAGE,damageType:"FIRE",visionBlock:false});
      events.push({type:"FIRE_TORNADO_CREATED",x,y,effect:EFFECT.FIRE_TORNADO});
    }
    ...
    if(raining&&forceSet.has(FORCE.FIRE)){
      ...
      addEffect(state,x,y,{type:EFFECT.BURNING,duration:3,lightRadius:2,damage:HAZARD.BURNING_DAMAGE,damageType:"FIRE",fireIntensity:"NORMAL"});
    ...
    if(raining&&(forceSet.has(FORCE.HEAVY_FIRE)||forceSet.has(FORCE.EXPLOSION))){
      ...
      addSteam(state,x,y,events,{duration:2,reason:forceSet.has(FORCE.HEAVY_FIRE)?"HEAVY_FIRE_IN_RAIN":"EXPLOSION_IN_RAIN"});
    }else if(!raining&&environment===ELEMENT.GRASS&&forceSet.has(FORCE.HEAVY_FIRE)){
      addEffect(state,x,y,{type:EFFECT.BURNING,duration:3,lightRadius:2,damage:HAZARD.BURNING_DAMAGE,damageType:"FIRE",fireIntensity:"HEAVY"});
      events.push({type:"IGNITE",x,y,effect:EFFECT.BURNING});
    }
    ...
    if(environment===ELEMENT.WATER){
      if(forceSet.has(FORCE.HEAVY_FIRE)){
        removeEffect(state,x,y,EFFECT.BURNING);
        heatWater(map,state,x,y,events);
    ...
    if(environment===ELEMENT.STONE&&forceSet.has(FORCE.EXPLOSION)){
      ...
      const object=objectAt(map,x,y),destroyed=destroyStoneObject(map,state,object);
  ...
  function createTornado(state,x,y,{duration=2,pushDistance=2,lift=3,damage=20,fireDamage=45,resistAxes={horizontal:false,vertical:true}}={}){
    ...
    if(burning){addEffect(state,x,y,{type:EFFECT.FIRE_TORNADO,duration,pushDistance,lift,resistAxes:{...resistAxes},lightRadius:3,damage:fireDamage,damageType:"FIRE",visionBlock:false});return {type:"FIRE_TORNADO_CREATED",x,y,effect:EFFECT.FIRE_TORNADO};}
    addEffect(state,x,y,{type:EFFECT.TORNADO,duration,pushDistance,lift,resistAxes:{...resistAxes},damage,damageType:"PHYSICAL",visionBlock:false});return {type:"TORNADO_CREATED",x,y,effect:EFFECT.TORNADO};
  }
  function pathInteraction({state,x,y,kind="UNIT"}={}){
    ...
    return {interrupted:true,effects:[{type:"FORCED_MOVE",effect:tornado,distance:Number(tornado.pushDistance||2),lift:Number(tornado.lift||0),resistAxes:tornado.resistAxes||{horizontal:false,vertical:true},damage:Number(tornado.damage||0)}]};
  }
  function tick(state){
    for(const [k,list] of [...state.effects.entries()]){
      const next=[];
      ...
      if(next.length)state.effects.set(k,next);else state.effects.delete(k);
    }
  }
  ...
  return{
    ELEMENT,FORCE,EFFECT,HAZARD,WEATHER,WEATHER_RULES,HYDROLOGY,
    create,setTimeOfDay,setWeather,isRain,advanceHydrology,lightningRisk,rollWeatherEvent,environmentAt,effectAt,isBurning,isBoiling,isConductive,conductiveRegion,conductThunder,
    elevation,waterDepth,fillCapacity,addWater,removeWater,deformTerrain,apply,createTornado,pathInteraction,tick,lightSources,isLit,visionModifier
  };
})();
