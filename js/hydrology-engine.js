window.HydrologyEngine=(()=>{
  ...
  function sync(tile,events=[]){
    ...
    tile.waterSurfaceZ=d>0?elevation(tile)+d:null;
    if(d>0&&tile.terrain!=="WATER"){
      tile.dryTerrain=tile.terrain==="MUD"?"PLAIN":tile.terrain;
      tile.terrain="WATER";
      events.push({type:"BASIN_FILLED",x:tile.x,y:tile.y,elevation:elevation(tile),waterDepth:d,waterSurfaceZ:tile.waterSurfaceZ});
    }else if(d<=0&&tile.terrain==="WATER"&&tile.dryTerrain){
      tile.terrain=tile.dryTerrain;
      delete tile.dryTerrain;
      events.push({type:"BASIN_DRAINED",x:tile.x,y:tile.y,elevation:elevation(tile),waterDepth:0});
    }
  }

  function setWaterDepth(tile,nextDepth,events=[],source="HYDROLOGY"){
    if(!tile)return 0;
    const before=waterDepth(tile),next=Math.max(0,Number(nextDepth||0));
  ...
  function addWater(tile,amount,events=[]){
    if(!tile)return 0;
    const before=waterDepth(tile),capacity=Math.max(fillCapacity(tile),before),next=Math.min(capacity,before+Math.max(0,Number(amount||0)));
  ...
  function floodArea(map,tiles,{surfaceRise=1,source="FLOOD"}={}){
    const events=[];
    const area=(tiles||[]).filter(tile=>tile&&tile.terrain!=="WALL");
    if(!area.length)return events;

    const existingSurfaces=area.map(waterSurfaceZ).filter(value=>value!=null&&Number.isFinite(value));
    const baseSurface=existingSurfaces.length
      ?Math.max(...existingSurfaces)
      :Math.min(...area.map(elevation));
    ...
    for(const tile of ordered){
      ...
      const required=Math.max(waterDepth(tile),targetSurface-elevation(tile));
      setWaterDepth(tile,required,events,source);
    }
    events.push({type:"FLOOD_AREA_RESOLVED",source,targetSurface,tiles:ordered.map(tile=>({x:tile.x,y:tile.y,elevation:elevation(tile),waterDepth:waterDepth(tile),waterSurfaceZ:waterSurfaceZ(tile)}))});
    return events;
  }
  ...
  function applyRain(map,{heavy=false}={}){
    ...
    for(const tile of map?.tiles||[]){
      if(elevation(tile)<WATERLINE||waterDepth(tile)>0)addWater(tile,amount,events);
    }
    return events;
  }
  ...
  return Object.freeze({
    ...
    setWaterDepth,addWater,removeWater,floodArea,deformTerrain,applyRain
  });
})();
