  ...
  function forcedMove({map,units,source,target,effect,_depth=0,_visited=new Set(),onCollision=null}){
    ...
    for(let i=0;i<distance;i++){
      ...
      if(surface){
        ...
        const hit=surface.unit,transfer=surface.kind==="UNIT"&&CollisionEngine.canTransfer(target,hit,remaining);let chain=null;
        if(transfer){const transferred=Math.max(1,remaining-1);chain=forcedMove({map,units,source:{x:target.x,y:target.y},target:hit,effect:{type:"KNOCKBACK",distance:transferred,lift:0,force:{horizontal:transferred,vertical:0},resistAxes:{horizontal:true,vertical:true}},_depth:_depth+1,_visited:visited,onCollision});if(hit?.alive)damageUnit(hit,Math.max(1,Math.round(damage*.5)))}
        ...
        const collision={kind:surface.kind,x:nx,y:ny,surface,damage,transferred:transfer,chain,objectDamage,objectDestroyed,stopped:!objectDestroyed};collisions.push(collision);onCollision?.(collision,{mover:target,source,effect,depth:_depth});
        if(objectDestroyed&&target.alive){TrajectoryEngine.advance(trajectory,inspection);target.x=nx;target.y=ny;target.z=trajectory.z;steps.push({x:nx,y:ny,z:trajectory.z,elevation:TacticalEngine.elevation(to),trajectoryState:trajectory.state,brokeObject:surface.object.id});continue}
        break;
      }
      TrajectoryEngine.advance(trajectory,inspection);target.x=nx;target.y=ny;target.z=trajectory.z;
      steps.push({x:nx,y:ny,z:trajectory.z,elevation:TacticalEngine.elevation(to),trajectoryState:trajectory.state});
    }
    const preLandingState=trajectory.state,landing=resolveLanding({map,target,fromZ:trajectory.z}),falls=landing.damaging?[{from:landing.fromZ,to:landing.toZ,drop:landing.drop}]:[];
    return{type:effect.type,applied:steps.length>0||lift>0||collisions.length>0,start,end:{x:target.x,y:target.y,z:target.z},steps,collisions,airborne:preLandingState!=="GROUNDED",trajectoryState:preLandingState,lift,travelZ,displacement,landing,falls,fallDamage:landing.damage,defeated:!target.alive};
  }
