window.PostEngagementEngine=(()=>{
  ...
  function openDistance(map,units,target,dir,distance){
    ...
    for(let i=0;i<distance;i++){
      ...
      if(TacticalEngine.elevationDelta(from,to)>1)break;
      x=nx;y=ny;total++;
    }
    return total;
  }
  function fallbackDirection(map,units,target,distance){
    return DIRS.map((d,index)=>({d,index,space:openDistance(map,units,target,d,distance)}))
  ...
  function forcedMove({map,units,source,target,effect}){
    ...
    if(!dir.dx&&!dir.dy){
      const fallback=fallbackDirection(map,units,target,distance);
      dir={dx:fallback[0],dy:fallback[1]};
    }
    if(!dir.dx&&!dir.dy)return {type:effect.type,applied:false,reason:"BLOCKED"};
    ...
    for(let i=0;i<distance;i++){
      ...
      if(!to||!TERRAINS[to.terrain]?.passable||occupied(units,nx,ny,target.id))break;
      const delta=TacticalEngine.elevationDelta(from,to);if(delta>1)break;
      target.x=nx;target.y=ny;steps.push({x:nx,y:ny,elevation:TacticalEngine.elevation(to)});
      if(delta<=-FALL_THRESHOLD)falls.push({from:TacticalEngine.elevation(from),to:TacticalEngine.elevation(to),drop:-delta});
    }
    let damage=0;for(const fall of falls)damage+=fallDamage(fall.drop);
