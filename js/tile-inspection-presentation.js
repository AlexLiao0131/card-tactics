window.TileInspectionPresentation=(()=>{
  ...
  function create({map,environmentState,tile}){
    ...
    const title=depth>0?`${terrain.name||tile.terrain}｜地面 H${ground}｜水面 H${surface}`:`${terrain.name||tile.terrain}｜H${ground}`;
    const meta=depth>0?`水深 ${depth}｜MOVE ${move}｜EVA ${eva}`:`MOVE ${move}｜EVA ${eva}`;
    const summary={title,meta,status:effects.map(e=>EFFECT[e.type]||e.type).join("・")};
    const details=[`地圖格 (${tile.x},${tile.y})`,`地形：${terrain.name||tile.terrain}｜地面 H${ground}`,`環境材質：${ENV[material]||material}｜天候：${WEATHER[environmentState?.weather]||environmentState?.weather||"晴朗"}`];
    if(depth>0)details.push(`水深：${depth}｜水面 H${surface}`);
    ...
    if(material==="WATER")details.push("互動：相連水體可傳導雷元素；火焰受水抑制；高熱可產生蒸氣與改變水量。");
