...
function render(){
  ...
  const visible=!!data&&!unit&&!hidden;el.classList.toggle("visible",visible);el.classList.toggle("expanded",visible&&expanded);if(!visible)return;
  el.querySelector("strong").textContent=data.summary.title;el.querySelector("span").textContent=data.summary.meta;el.querySelector("small").textContent=data.summary.status||"點擊展開詳細資訊";el.querySelector("pre").textContent=data.details;
}
